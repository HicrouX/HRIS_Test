...
if (!isset($_SESSION['role_id']) || ($_SESSION['role_id'] != 3 && $_SESSION['role_id'] != 4)) { header("Location: login.php"); exit; }
...
<body>
    ...
    <div class="main-content">
        <div id="master-view" class="view-content active-view">
            <div class="header">
                <div><h2 style="margin:0;">📊 Master Attendance</h2></div>
            ...
            <div class="container"><table id="masterTable"><thead id="masterTableHead"><tr><th>Personnel</th><th>Last Active</th><th>Status</th><th>Actions</th></tr></thead><tbody id="masterLogsBody"></tbody></table></div>
        </div>
        <div id="approvals-view" class="view-content">
            <div class="header"><h2>✅ Final Sign-offs</h2></div>
            <div class="container">
                <h3>Leaves</h3><table id="adminLeaveTable"><thead><tr><th>Employee</th><th>Date Range</th><th>Reason</th><th>Endorsed By</th><th>Action</th></tr></thead><tbody id="adminLeaveQueue"></tbody></table>
                <h3 style="margin-top:40px;">Overtime</h3><table id="adminOTTable"><thead><tr><th>Employee</th><th>Date/Time</th><th>Purpose</th><th>Endorsed By</th><th>Action</th></tr></thead><tbody id="adminOTQueue"></tbody></table>
                <h3 style="margin-top:40px; color:#e74c3c;">Attendance Disputes (Pending)</h3>
                <table id="adminDisputeTable">
                    <thead><tr><th>Employee</th><th>Type</th><th>Reason</th><th>Action</th></tr></thead>
                    <tbody id="adminDisputeQueue"></tbody>
                </table>
            </div>
        </div>
        <div id="history-view" class="view-content">
            <div class="header"><h2 style="margin:0;">📜 Request History</h2><div style="display:flex; gap:10px; align-items:center;"><select id="hist_status" style="padding:8px; border-radius:5px;"><option value="ALL">All Status</option><option value="Pending">Pending</option><option value="Endorsed">Endorsed</option><option value="Approved">Approved</option><option value="Denied">Denied</option></select><input type="date" id="hist_start" class="date-input-small"><span style="color:white;">to</span><input type="date" id="hist_end" class="date-input-small"><button class="export-btn" onclick="loadRequestHistory()">Filter</button></div></div>
            <div class="container"><table id="historyTable"><thead><tr><th>Employee</th><th>Type</th><th>Details</th><th>Date Range</th><th>Status</th><th>Filed On</th></tr></thead><tbody id="historyBody"></tbody></table></div>
        </div>
        <div id="my-requests-view" class="view-content"><div class="header" style="background: #8e44ad;"><h2 style="margin:0;">My Request Status</h2><button class="export-btn" onclick="loadMyRequests()">🔄 Refresh</button></div><div class="container"><h3 style="color:#666;">My Leave Requests</h3><table><thead><tr><th>Type</th><th>Date Range</th><th>Reason</th><th>Status</th><th>Filed On</th><th>Approved By</th></tr></thead><tbody id="myLeaveLogs"></tbody></table><h3 style="color:#666; margin-top:40px;">My Overtime Requests</h3><table><thead><tr><th>Type</th><th>Time Range</th><th>Purpose</th><th>Status</th><th>Filed On</th><th>Approved By</th></tr></thead><tbody id="myOTLogs"></tbody></table><h3 style="color:#666; margin-top:40px;">My Disputes</h3><table><thead><tr><th>Type</th><th>Date</th><th>Reason</th><th>Status</th><th>Filed On</th></tr></thead><tbody id="myDisputeLogs"></tbody></table></div></div>
        <div id="admin-filing" class="view-content"><div class="header" style="background: #3498db;"><h2>Filing Center</h2></div><div class="container"><div class="form-card"><h3>📝 Leave</h3><form id="leaveForm" class="form-grid"><select id="l_type"><option>Sick Leave</option><option>Vacation Leave</option></select><div></div><input type="date" id="l_start"><input type="date" id="l_end"><textarea id="l_reason" placeholder="Reason..."></textarea><button type="button" class="submit-btn" onclick="submitRequest('leave')">Submit</button></form></div><div class="form-card" style="border-left: 5px solid #27ae60;"><h3>⏰ Overtime</h3><form id="otForm" class="form-grid"><select id="ot_type"><option>Regular Overtime</option><option>Duty on Rest Day</option></select><div></div><input type="datetime-local" id="ot_start"><input type="datetime-local" id="ot_end"><textarea id="ot_purpose" placeholder="Purpose..."></textarea><button type="button" class="submit-btn" style="background:#27ae60;" onclick="submitRequest('ot')">Submit</button></form></div>
        ...
        <div id="my-attendance" class="view-content"><div class="header"><div style="display:flex; align-items:center; gap:10px;"><button class="export-btn" onclick="exportData('MY')">📂 Export Excel</button><h2 style="margin:0;">Attendance History</h2></div><div style="display:flex;"><input type="date" id="my_start" onchange="loadMyAttendance()"><input type="date" id="my_end" style="margin-left:5px;" onchange="loadMyAttendance()"></div></div><div class="container"><table><thead><tr><th>Date</th><th>In</th><th>Out</th><th>Status</th><th>Hrs</th></tr></thead><tbody id="myAttendanceBody"></tbody></table></div></div>
    </div>
    ...
    <script>
        ...
        function exportCurrentModalUser() { const id = document.getElementById('current_view_id').value; if(id) exportSingleEmployee(id); }
        ...
        async function submitRequest(type) {
            ...
            if (type === 'leave') { endpoint = '/users/file_leave.php'; formId = 'leaveForm'; payload = { employee_id: MY_ID, leave_type: document.getElementById('l_type').value, start_date: document.getElementById('l_start').value, end_date: document.getElementById('l_end').value, reason: document.getElementById('l_reason').value, agreement_1: 1, agreement_2: 1 }; }
            else if (type === 'ot') {
                const start = new Date(document.getElementById('ot_start').value); const end = new Date(document.getElementById('ot_end').value);
                const diffHrs = (end - start) / (1000 * 60 * 60); if (diffHrs > 2) { alert("⚠️ Cannot submit: Overtime is limited to 2 hours per request."); return; }
                endpoint = '/users/file_overtime.php'; formId = 'otForm'; payload = { employee_id: MY_ID, ot_type: document.getElementById('ot_type').value, start_time: document.getElementById('ot_start').value, end_time: document.getElementById('ot_end').value, purpose: document.getElementById('ot_purpose').value, agreement_1: 1, agreement_2: 1 };
            ...
            try { const res = await fetch(`${API}${endpoint}`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload) }); const result = await res.json(); alert(result.success || result.error); if(result.success) { document.getElementById(formId).reset(); loadMyRequests(); } } catch (err) { alert("Submission failed."); }
        }
